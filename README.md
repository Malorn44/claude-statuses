...
- **`scripts/fetch.mjs`** — cron-driven poll (daily at 04:17 UTC). Pulls the public Statuspage endpoints + the homepage HTML, merges new incident updates into `data/incidents.jsonl` idempotently (one minified JSON record per line, sorted by `started_at`), and refreshes the rest of `data/`.
...
| `data/uptime-history.json` | `/uptime/<componentId>.json?page=N` | Per-component, paginated 3 months at a time back to 2021. By default `fetch.mjs` only fetches **page 1** (the ~3 most recent months) and merges it into the existing file — older months pass through untouched. With `FULL_HISTORY=1`, walks all pages until empty (~20 pages × 6 components on first seed). | Page 1 every poll; full back-walk weekly via the `FULL_HISTORY=1` cron entry. |
